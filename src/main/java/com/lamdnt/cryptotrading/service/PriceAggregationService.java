...
public class PriceAggregationService {
  ...
  public Map<String, Pair<BigDecimal, BigDecimal>> getAggregationPrice(List<String> symbols) {
    ...
    Map<String, BinanceTickerResponse> binancePrices =
        binanceHttpClient.getTickers(httpHelper.buildParam(symbols)).stream()
    ...
    Map<String, HuobiTickerResponse.HuobiTicker> huobiPrices =
        huobiHttpClient.getTickers().getData().stream()
            .filter(ticker -> symbols.contains(ticker.getSymbol().toUpperCase()))
            .collect(
                Collectors.toMap(ticker -> ticker.getSymbol().toUpperCase(), ticker -> ticker));
    ...
    return symbols.stream()
        .map(
            symbol -> {
              ...
              BigDecimal bestAsk =
                  minPrice(
                      Optional.ofNullable(binance)
                          .map(BinanceTickerResponse::getAskPrice)
                          .orElse(null),
                      Optional.ofNullable(huobi)
                          .map(HuobiTickerResponse.HuobiTicker::getAsk)
                          .orElse(null));

              BigDecimal bestBid =
                  maxPrice(
                      Optional.ofNullable(binance)
                          .map(BinanceTickerResponse::getBidPrice)
                          .orElse(null),
                      Optional.ofNullable(huobi)
                          .map(HuobiTickerResponse.HuobiTicker::getBid)
                          .orElse(null));
